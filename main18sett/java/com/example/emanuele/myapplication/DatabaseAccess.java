...
public class DatabaseAccess {
    ...
    public List<PointF> getMarksPassaggio(String piano) {
        List<PointF> marks = new ArrayList<>();
        Cursor cursor=database.rawQuery("SELECT coordinata_x,coordinata_y from Associato WHERE piano=?",new String[] { piano });
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            float x=cursor.getFloat(0);
            float y=cursor.getFloat(1);
    ...
    public void getMarksPassaggioCod(PointF start) {//serve per trovare la posizione di partenza quando l utente all inizio non si trova sul piano corretto
        ...
        List<PointF> marks = new ArrayList<>();
        Cursor cursor=database.rawQuery("SELECT passaggio from Associato WHERE coordinata_x=? and coordinata_y=?",new String[] {String.valueOf(start.x),String.valueOf(start.y)});
        cursor.moveToFirst();
        String cod=cursor.getString(0);
        cursor.close();
        Cursor cursor2=database.rawQuery("SELECT coordinata_x,coordinata_y from Associato WHERE piano=? and passaggio=?",new String[] { p.getDestinationPiano(),cod });
        cursor2.moveToFirst();
